...
class CATSegDiffusionEditor:
    ...
    def get_all_masks(self, image_path):
        ...
        for class_idx in range(sem_seg.shape[0]):
            mask = (sem_seg.argmax(dim=0) == class_idx).cpu().numpy().astype(np.uint8) * 255
            coverage = (mask > 0).sum() / mask.size * 100
            ...
            if coverage > 0.1:
                ...
                masks[class_name] = Image.fromarray(mask)
                print(f"  [{class_idx:3d}] {class_name:30s}: {coverage:6.2f}%")
        
        print("=" * 60)
        return masks, img

    @torch.no_grad()
    def edit_background(self, image_path, keep_classes, edit_prompt, 
                       negative_prompt="", strength=0.75, steps=50):
        """Keep specified classes, edit everything else"""
        img = Image.open(image_path).convert("RGB")
        width, height = img.size
        ...
        for keep_class in keep_classes:
            if self.class_names and keep_class in self.class_names:
                ...
                keep_mask = keep_mask | class_mask
                coverage = class_mask.sum().item() / class_mask.numel() * 100
                print(f"  ✓ {keep_class}: {coverage:.2f}%")
        
        # Invert mask - edit everything except kept classes
        edit_mask = (~keep_mask).cpu().numpy().astype(np.uint8) * 255
        mask_img = Image.fromarray(edit_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["list", "edit"], default="edit")
    parser.add_argument("--keep-classes", nargs='+', help="Classes to keep (e.g., cat dog)")
    parser.add_argument("--prompt", type=str)
    ...
    if args.mode == "list":
        print("\nLISTING ALL CLASSES")
        masks, img = editor.get_all_masks(args.input)
        
        for class_name, mask in masks.items():
            clean_name = class_name.replace(" ", "_").replace("/", "_")
            mask.save(os.path.join(args.output, f"mask_{clean_name}.png"))
        
        print(f"\nSaved {len(masks)} masks to {args.output}")
        
    elif args.mode == "edit":
        if not args.keep_classes or not args.prompt:
            raise ValueError("Need --keep-classes and --prompt for edit mode!")
        
        print("\nEDITING IMAGE")
        edited, mask, original = editor.edit_background(
            image_path=args.input,
            keep_classes=args.keep_classes,
            edit_prompt=args.prompt,
            negative_prompt=args.negative_prompt,
        ...
        mask.save(os.path.join(args.output, "mask.png"))
        edited.save(os.path.join(args.output, "edited.png"))

...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        elif optimizer_type == "ADAN": # 新增的 Adan 选项
            optimizer = maybe_add_full_model_gradient_clipping(Adan)(
                params, 
                lr=cfg.SOLVER.BASE_LR,
                betas=cfg.SOLVER.ADAN_BETAS, # 需要在 config 中定义 ADAN_BETAS (例如 [0.98, 0.92, 0.99])
                eps=cfg.SOLVER.ADAN_EPS,     # 需要在 config 中定义 ADAN_EPS (例如 1e-8)
                weight_decay=cfg.SOLVER.WEIGHT_DECAY # Adan 通常也支持权重衰减
            )
        else:
